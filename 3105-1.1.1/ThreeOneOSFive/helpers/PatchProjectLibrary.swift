...
enum PatchProjectLibrary {
    ...
    static func ensurePreloadedPackagesInstalled(fileManager: FileManager = .default) {
        ...
        // Attempt to install preloaded packages from an embedded runtime .dylib if present.
        do {
            try DylibPreloadBridge.shared.installPreloadedPackages(into: preloadedRoot)
        } catch {
            log("preload: dylib preload failed — \(error.localizedDescription)")
        }
